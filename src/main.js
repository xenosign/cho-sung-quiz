...
function renderQuizScreen() {
    ...
    <div class="quiz-card">
      ...
      <div class="team-grid">
        ${teamScores
          .map(
            (_, i) =>
              `<button class="team-btn" data-team="${i}">${i + 1}팀</button>`,
          )
          .join('')}
      </div>
  ...
  app
    ...
    .addEventListener('click', renderCategoryScreen);
}
...
function getFamilyQuestionPool() {
  return familyCategories.flatMap((c) =>
    c.words.map((word) => ({ category: c.name, word })),
  );
}
...
function renderFamilyRoundSelectScreen() {
  ...
  app.querySelectorAll('.round-btn').forEach((btn) => {
    btn.addEventListener('click', () =>
      startFamilyRound(Number(btn.dataset.round)),
    );
  });
...
function renderFamilyRoundScreen() {
  ...
  app
    ...
    .addEventListener('click', goToNextFamilyQuestion);
